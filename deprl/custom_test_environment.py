...
def get_torque_cost(env, actions):
    addresses = np.where(np.array([env.sim.model.actuator(i).gaintype[0] for i in range(env.sim.model.nu)]) == 0)[0]
...
def test_mujoco(env, agent, steps, params=None, test_episodes=10):
    ...
    for _ in range(test_episodes):
        ...
        while True:
            ...
            ref_error = env.environments[0].obs_dict["robot_error"].reshape(-1, 3)
            metrics["test/tracking_error"] += np.mean(np.square(ref_error))
            metrics["test/torque_cost"] += get_torque_cost(env.environments[0], actions)
            if eval_rwd_metrics:
        ...
        if eval_rwd_metrics:
            metrics["test/tracking_error"] /= metrics["test/episode_length"]
        ...
        logger.store('test/jetpack_curriculum_coef', env.environments[0].unwrapped.jetpack_curriculum._value)
        logger.store('test/spline_curriculum_coef', env.environments[0].unwrapped.spline_curriculum._value)
        logger.store('test/terrain_curriculum_coef', env.environments[0].unwrapped.terrain_curriculum._value)
        env.environments[0].jetpack_curriculum.update(-metrics["test/tracking_error"])
        env.environments[0].spline_curriculum.update(metrics["test/episode_length"])
        env.environments[0].terrain_curriculum.update(metrics["test/episode_length"])
    return metrics
